...
import { getEthereumAddressById, updateEthereumAddressById } from 'apiSdk/ethereum-addresses';
...
function EthereumAddressEditPage() {
  ...
  const handleSubmit = async (values: EthereumAddressInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateEthereumAddressById(id, values);
      mutate(updated);
      resetForm();
